...
export function Header({ activeView, onNavigate, onOpenMobileSidebar }: HeaderProps) {
  ...
  return (
    <header className="sticky top-0 z-20 flex h-16 w-full items-center justify-between border-b border-slate-200/80 dark:border-white/10 bg-white/85 dark:bg-[#070B14]/85 px-3 sm:px-6 backdrop-blur-xl transition-colors">
      <div className="flex items-center gap-2.5 sm:gap-3 min-w-0">
        {/* Mobile menu button */}
        <button
          onClick={onOpenMobileSidebar}
          className="lg:hidden h-10 w-10 shrink-0 flex items-center justify-center rounded-xl text-slate-700 dark:text-slate-300 hover:text-slate-950 dark:hover:text-white hover:bg-slate-200/60 dark:hover:bg-slate-800/80 active:bg-slate-300 dark:active:bg-slate-700 transition-colors border border-slate-300/60 dark:border-white/5"
        ...
        <div className="truncate">
          <div role="heading" aria-level={2} className="text-sm font-bold text-slate-900 dark:text-white sm:text-base leading-none truncate">
            {breadcrumb.title}
          </div>
          <p className="hidden sm:block text-xs text-slate-500 dark:text-slate-400 mt-1 truncate">
            {breadcrumb.subtitle}
          </p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 sm:gap-3 shrink-0">
        ...
        {activeView !== 'curriculum-hub' && (
          <button
            ...
            className="hidden sm:inline-flex items-center gap-1.5 rounded-xl border border-slate-200 dark:border-white/10 bg-slate-100/90 dark:bg-slate-900/90 px-3 py-1.5 text-xs font-bold text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-800 hover:text-slate-950 dark:hover:text-white transition-colors shadow-xs"
        ...
        {errorCount > 0 && (
          <button
            ...
            className="flex items-center gap-1.5 rounded-xl border border-amber-500/30 bg-amber-500/10 px-2.5 py-1.5 text-xs font-semibold text-amber-700 dark:text-amber-300 hover:bg-amber-500/20 active:bg-amber-500/30 transition-colors"
