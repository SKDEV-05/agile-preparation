...
export function SectionArticle({ section, isCompleted, onMarkCompleted }: SectionArticleProps) {
  ...
  return (
    <article id={section.id} className="reveal-on-scroll scroll-mt-24 border-b border-white/10 pb-12 mb-12 text-slate-200">
      {/* Section Header */}
      <div className="flex items-center gap-3">
        <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-indigo-500/20 border border-indigo-500/30 font-mono font-bold text-xs text-indigo-400 shrink-0">
          {section.order}
        </span>
      ...
      {/* 1. Définition Card */}
      <div className="mt-5 rounded-2xl border border-indigo-500/30 bg-gradient-to-r from-indigo-950/40 via-slate-900/60 to-slate-900/60 p-4 sm:p-5 shadow-lg backdrop-blur-md">
        <div className="flex items-center gap-2 text-xs font-mono font-bold uppercase tracking-wider text-indigo-300">
          <BookmarkCheck className="h-4 w-4 shrink-0 text-indigo-400" />
      ...
      {section.deepExplanation && (
        <div className="mt-6 rounded-2xl border border-white/10 bg-slate-900/70 p-5 shadow-md">
          <h3 className="text-xs font-bold uppercase tracking-wider text-indigo-400 flex items-center gap-2">
            <BookOpen className="h-4 w-4" />
            <span>Explication Approfondie · Clés de Maîtrise</span>
          </h3>
          <p className="mt-2.5 text-xs sm:text-sm text-slate-300 leading-relaxed font-normal whitespace-pre-line">
            {section.deepExplanation}
          </p>
        </div>
      )}
        ...
        <div className="mt-6">
          ...
          <div className="mt-2.5 space-y-2">
            {section.examples.map((ex, idx) => (
              <div key={idx} className="flex items-start gap-3 rounded-xl border border-white/5 bg-slate-900/60 p-3.5 shadow-sm text-xs sm:text-sm text-slate-300 leading-relaxed">
                <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-md bg-slate-800 text-slate-400 font-mono font-bold text-[11px]">
                  {idx + 1}
                </span>
                <span>{ex}</span>
              </div>
      ...
      {section.realWorldCase && (
        <div className="mt-7 rounded-3xl border border-cyan-500/30 bg-gradient-to-br from-cyan-950/30 via-slate-900/80 to-indigo-950/30 p-5 sm:p-6 shadow-xl">
          <div className="flex items-center justify-between pb-3 border-b border-cyan-500/20">
            <div className="flex items-center gap-2 text-xs font-bold text-cyan-300 uppercase tracking-wider">
              <Building2 className="h-4 w-4 text-cyan-400" />
              <span>Cas Réel d'Entreprise · {section.realWorldCase.company}</span>
            </div>
            <span className="rounded-full bg-cyan-500/20 border border-cyan-500/30 px-2.5 py-0.5 text-[10px] font-bold text-cyan-300 font-mono">
          ...
          <div className="mt-4 space-y-3 text-xs sm:text-sm">
            <div>
              <span className="font-bold text-white">Le Défi rencontré : </span>
              <span className="text-slate-300">{section.realWorldCase.problem}</span>
            </div>
            <div>
              <span className="font-bold text-cyan-300">Solution Agile adoptée : </span>
              <span className="text-slate-300">{section.realWorldCase.agileSolution}</span>
            </div>
            <div className="rounded-xl bg-cyan-950/50 border border-cyan-500/30 p-3 text-cyan-200 font-semibold text-xs flex items-start gap-2">
      ...
      <div className="mt-8 grid sm:grid-cols-2 gap-4">
        {/* À retenir */}
        <div className="rounded-2xl border border-emerald-500/30 bg-emerald-950/20 p-4 sm:p-5 shadow-md">
        ...
        {/* Attention / Pièges */}
        <div className="rounded-2xl border border-amber-500/30 bg-amber-950/20 p-4 sm:p-5 shadow-md">
      ...
      {section.examContext && (
        <div className="mt-6 flex items-start gap-3 rounded-2xl border border-indigo-500/20 bg-indigo-950/30 p-4 shadow-sm text-xs sm:text-sm text-slate-300">
          <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-indigo-500/20 text-indigo-400 mt-0.5 border border-indigo-500/30">
            <Lightbulb className="h-4 w-4" />
          </div>
          <div className="leading-relaxed">
            <span className="font-bold text-white">Astuce Examen OFPPT : </span>
            {section.examContext}
          </div>
        </div>
      )}
      ...
      {section.miniQuestion && (
        <div className="mt-8 rounded-2xl border border-white/10 bg-slate-900/80 p-5 sm:p-6 shadow-xl">
          ...
          <div className="mt-4 space-y-2.5">
            {section.miniQuestion.options.map((opt, idx) => {
              ...
              if (!isMiniAnswerChecked) {
                btnCls += "border-white/5 bg-slate-800/60 hover:bg-slate-800 hover:border-indigo-500/40 text-slate-200";
              } else if (isCorrect) {
                btnCls += "border-emerald-500/50 bg-emerald-950/40 text-emerald-200 font-semibold";
              } else if (isSelected && !isCorrect) {
                btnCls += "border-red-500/50 bg-red-950/40 text-red-200 animate-error-shake";
              } else {
                btnCls += "border-white/5 bg-slate-900/40 text-slate-500 opacity-40";
              }
              ...
              return (
                <button
                  ...
                  className={btnCls}
                >
                  <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-md bg-slate-800 border border-white/10 text-slate-300 font-mono font-bold text-xs mt-0.5">
      ...
      <div className="mt-6 flex justify-end">
        ...
        >
          <CheckCircle2 className={`h-4 w-4 ${isCompleted ? 'text-emerald-400' : 'text-slate-500'}`} />
