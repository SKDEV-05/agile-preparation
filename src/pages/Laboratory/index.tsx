...
export function LaboratoryPage() {
  ...
  return (
    <div className="space-y-8 pb-12">
      {/* Page Header */}
      <div className="reveal-on-scroll relative rounded-3xl border border-slate-200 dark:border-0 ring-1 ring-cyan-500/20 bg-gradient-to-b from-slate-50 via-white to-slate-100 dark:from-[#0F172A]/95 dark:via-[#0D1526]/90 dark:to-[#070B14]/95 backdrop-blur-2xl p-5 sm:p-8 shadow-sm dark:shadow-[0_25px_60px_-15px_rgba(0,0,0,0.85),0_0_40px_-10px_rgba(6,182,212,0.18)] overflow-hidden">
        ...
        <div className="flex items-center gap-1.5 mb-4 pb-3 border-b border-slate-200 dark:border-white/5">
          <div className="w-2.5 h-2.5 rounded-full bg-red-500/80"></div>
          <div className="w-2.5 h-2.5 rounded-full bg-amber-500/80"></div>
          <div className="w-2.5 h-2.5 rounded-full bg-emerald-500/80"></div>
          <span className="ml-2 font-mono text-[10px] text-slate-500 dark:text-slate-400">fullstack2a@laboratory: ~/simulators</span>
        </div>

        <div className="relative z-10 flex items-center gap-3">
          <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-cyan-500/10 dark:bg-cyan-600/20 border border-cyan-500/20 dark:border-cyan-500/30 text-cyan-600 dark:text-cyan-400 shadow-sm dark:shadow-lg shrink-0">
        ...
        <p className="relative z-10 mt-3 text-xs sm:text-sm text-slate-600 dark:text-slate-300 max-w-3xl leading-relaxed">
        ...
        <div className="relative z-10 mt-6 flex overflow-x-auto no-scrollbar sm:flex-wrap gap-2 border-t border-slate-200 dark:border-white/5 pt-4 pb-1 sm:pb-0">
          {tabs.map((tab) => {
            const Icon = tab.icon;
            ...
            return (
              <button
                ...
                className={`shrink-0 flex items-center gap-2 rounded-xl px-3.5 sm:px-4 py-2 sm:py-2.5 text-xs font-bold transition-all ${
                  isActive
                    ? 'bg-gradient-to-r from-cyan-600 to-indigo-600 text-white shadow-md dark:shadow-lg dark:shadow-cyan-600/30 scale-[1.02] border border-cyan-400/40'
                    : 'bg-slate-100 hover:bg-slate-200/80 text-slate-700 hover:text-slate-900 dark:bg-[#070B14]/80 dark:text-slate-400 dark:hover:bg-white/5 dark:hover:text-slate-200 border border-slate-200 dark:border-white/5'
                }`}
              >
                <Icon className={`h-4 w-4 ${isActive ? 'text-cyan-100' : 'text-slate-500'}`} />
                <span>{tab.label}</span>
              </button>
            );
          })}
